  ...
  sub evaluate_algorithm_cross_validation_split{
    ...
    while (my ($i, $fold) = each @folds){
      ...
      if (ref($dataset) eq 'AI::MXNet::NDArray'){
        ...
        ($predicted, $train_loss, $test_loss) = $algorithm->('sml', $train_set, $test_set, @_);
        ...
        for my $row (@$fold){
          my @row_copy  = @$row;
          push @test_set, [@row_copy];
          # $row_copy[-1] = undef; # Commented because in Backpropagation algorithm, the test labels are needed for measuring the loss and plotting.
        }  
        ($predicted, $train_loss, $test_loss) = $algorithm->('sml', \@train_set, \@test_set, @_);
        $actual = [map {$_->[-1]} @$fold];
      }
